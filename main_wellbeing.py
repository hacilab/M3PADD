...
import tqdm
import numpy as np
from Data_processing_wel_eeg import loading_data
from Data_processing_wel_emg import loading_data2
...
def remove_low_accuracy_folders(model_save_path):
    ...
    for folder in folders:
        try:
            ...
            acc_str = folder.split('_acc_')[1].split('_')[0]
            f1_str = folder.split('_f1_')[1].split('_')[0]
            acc = float(acc_str)
            f1 = float(f1_str)
            folder_acc_f1_map[folder] = (acc, f1)
        except (IndexError, ValueError) as e:
            print(f"No  {folder} ，error: {e}")
            continue
...
base_model_save_path = r"D:\our\wel_eeg+emg_zblt_per"
...
for lr in learning_rates:
    ...
    for iteration in range(1, 101):
        ...
        saved_model = torch.load(
            "Pretrained_weights/wellbeing_model.pt")
