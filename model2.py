...
class MULTAV_CLASSFICATIONModel1(nn.Module):
    def __init__(self):
        ...
        self.TAU_CBAM_eeg = TAU_CBAM(gate_channels=4, reduction_ratio=16, pool_types=['avg', 'max'])

        self.MultiheadAttention_event_before_eeg = nn.MultiheadAttention(embed_dim=self.hidden_dim2,
                                                                         num_heads=self.num_heads,
                                                                         dropout=self.out_dropout1, batch_first=True)
        self.MultiheadAttention_event_after_eeg = nn.MultiheadAttention(embed_dim=self.hidden_dim2,
                                                                        num_heads=self.num_heads,
                                                                        dropout=self.out_dropout1, batch_first=True)

        self.TAU_CBAM_emg = TAU_CBAM(gate_channels=4, reduction_ratio=16, pool_types=['avg', 'max'])

        self.MultiheadAttention_event_before_emg = nn.MultiheadAttention(embed_dim=self.hidden_dim2,
                                                                         num_heads=self.num_heads,
                                                                         dropout=self.out_dropout1, batch_first=True)
        self.MultiheadAttention_event_after_emg = nn.MultiheadAttention(embed_dim=self.hidden_dim2,
                                                                        num_heads=self.num_heads,
                                                                        dropout=self.out_dropout1, batch_first=True)
    ...
    def forward(self, x_z_eeg, x_b_eeg, x_l_eeg, x_t_eeg, x_z_emg, x_b_emg, x_l_emg, x_t_emg):
        ...
        event_before_eeg = torch.stack([x_z_eeg, x_b_eeg, x_l_eeg, x_t_eeg], dim=1)
        event_after_eeg = self.TAU_CBAM_eeg(event_before_eeg)


        x_m_before_eeg, _ = self.MultiheadAttention_event_before_eeg(event_after_eeg, event_before_eeg,
                                                                     event_before_eeg)
        x_m_after_eeg, _ = self.MultiheadAttention_event_after_eeg(event_before_eeg, event_after_eeg, event_after_eeg)

        x_b_eeg = event_before_eeg + x_m_before_eeg
        x_a_eeg = event_after_eeg + x_m_after_eeg

        x_eeg = x_b_eeg + x_a_eeg

        event_before_emg = torch.stack([x_z_emg, x_b_emg, x_l_emg, x_t_emg], dim=1)
        event_after_emg = self.TAU_CBAM_emg(event_before_emg)


        x_m_before_emg, _ = self.MultiheadAttention_event_before_emg(event_after_emg, event_before_emg,
                                                                     event_before_emg)
        x_m_after_emg, _ = self.MultiheadAttention_event_after_emg(event_before_emg, event_after_emg, event_after_emg)

        x_b_emg = event_before_emg + x_m_before_emg
        x_a_emg = event_after_emg + x_m_after_emg

        x_emg = x_b_emg + x_a_emg
        ...
        #
        b = torch.cat([x_eeg, x_m_eeg], dim=2)
        c = torch.cat([x_emg, x_m_emg], dim=2)

        x = torch.cat([b, c], dim=2)
        ...
        return output_depression, output_Ex, output_Ag, output_Co, output_Ne, output_Op
